"""Video slicing page: preview, clips, timeline, transport, export."""
...
class SlicingPage(QWidget):
    """Video slicing page with preview, clips, timeline, waveform, and export.

    Designed to be used as a tab inside MainWindow (the thin shell).
    The shell injects settings and calls setup_menus / install_shortcuts
    ...
    # ── public API called by MainWindow shell ──────────────────────

    def setup_menus(self, menu_bar: QMenuBar) -> None:
        """Build File and Edit menus on the shell's menu bar."""
        m_file = menu_bar.addMenu(tr("menu.file"))
    ...
    def teardown_menus(self) -> None:
        """Release QAction references so the shell can rebuild menus."""
        self._act_export = None
        self._act_export_strong = None
        self._act_load_subtitle = None
        self._act_unload_subtitle = None
        self._act_undo = None
        self._act_redo = None
    ...
    def install_shortcuts(self) -> None:
        ...
        for key, fn in (
            ...
            _bind(QKeySequence(key), Qt.ShortcutContext.WindowShortcut,
                  self._guard_no_text_focus(fn))
        ...
        for seq in (
            ...
            _bind(seq, Qt.ShortcutContext.WindowShortcut, lambda: self._pan_timeline(move_left=True))
        for seq in (
            QKeySequence("."),
            QKeySequence(Qt.Key.Key_Period),
        ):
            _bind(seq, Qt.ShortcutContext.WindowShortcut, lambda: self._pan_timeline(move_left=False))
        for seq in (
            QKeySequence("-"),
            QKeySequence(Qt.Key.Key_Minus),
        ):
            _bind(seq, Qt.ShortcutContext.WindowShortcut, lambda: self._zoom_timeline_from_shortcut(zoom_in=False))
        for seq in (
            QKeySequence("="),
            QKeySequence("+"),
            ...
            _bind(seq, Qt.ShortcutContext.WindowShortcut, lambda: self._zoom_timeline_from_shortcut(zoom_in=True))
        for seq in (
            QKeySequence("0"),
            QKeySequence(Qt.Key.Key_0),
            QKeySequence("Num+0"),
        ):
            _bind(seq, Qt.ShortcutContext.WindowShortcut, self._reset_timeline_view)
        _bind(
    ...
    def uninstall_shortcuts(self) -> None:
        """Remove all page shortcuts so they don't fire when tab is inactive."""
    ...
    @staticmethod
    def _guard_no_text_focus(fn: Any) -> Any:
        """Return a wrapper that only calls *fn* if no text input has focus."""
        def _wrapper() -> None:
            if not SlicingPage._is_text_input_focused():
                fn()
        return _wrapper

    def cleanup_on_close(self) -> None:
        """Stop all background threads and save project. Called by shell's closeEvent."""
        _app = QApplication.instance()
        if _app is not None:
            try:
    ...
    def on_tab_activated(self) -> None:
        """Called by shell when this tab becomes active."""
    ...
    def on_tab_deactivated(self) -> None:
        """Called by shell when this tab becomes inactive."""
    ...
    def _refresh_min_window_size_shell(self) -> None:
        """Trigger the parent window to recalculate its minimum size."""
        win = self.window()
        if isinstance(win, QMainWindow):
            mb = win.menuBar()
            mb_h = mb.sizeHint().height() if mb.isVisible() else 0
            win.setMinimumSize(750, 400 + mb_h)
    ...
    def refresh_language(self) -> None:
        ...
        Called by the shell after it rebuilds the menu bar and window title.
        """
        # Transport buttons
        self._apply_transport_button_icons()
        ...
        if event.type() == QEvent.Type.NativeGesture:
            ...
            if isinstance(_nge, _NGE) and self._data.source_path:
                ...
                if gt == Qt.NativeGestureType.EndNativeGesture:
                    ...
                    return True
        # ---- end pinch-to-zoom ----
        if isinstance(event, QKeyEvent) and self.isVisible():
    ...
    def _start_keyframe_scan(self, session_gen: int, *, show_status: bool) -> None:
        ...
        if show_status:
            self.status_message.emit(tr("status.analyzing_keyframes"))
    ...
    def show_settings_dialog(self) -> None:
        ...
        self._refresh_min_window_size_shell()
    ...
    def _load_subtitle_path(self, sub_path: Path) -> None:
        ...
        self._save_project()
        self.status_message.emit(tr("subtitle.status_loaded", filename=sub_path.name))
    ...
    def _unload_subtitle(self) -> None:
        ...
        self._act_unload_subtitle.setEnabled(False)
        self._save_project()
        self.status_message.emit(tr("subtitle.status_unloaded"))
    ...
    def _auto_detect_subtitle(self, video_path: Path) -> None:
        ...
        if candidate is not None:
            ...
            self._act_unload_subtitle.setEnabled(True)
            self._save_project()
            self.status_message.emit(tr("subtitle.status_loaded", filename=candidate.name))
    ...
    def _load_video_path(self, source: Path) -> None:
        ...
        if self._keyframes_deferred:
            self.status_message.emit(tr("status.keyframes_deferred"))
        else:
        ...
        if self._data.subtitle_path and self._data.subtitle_enabled:
            ...
            if Path(self._data.subtitle_path).is_file():
                ...
                if track is not None:
                    ...
                    self._video_preview_widget.set_subtitle_track(track)
                    self._act_unload_subtitle.setEnabled(True)
                    self.status_message.emit(tr("subtitle.status_loaded", filename=Path(self._data.subtitle_path).name))
                else:
    ...
    def _start_proxy_build(self) -> None:
        ...
        self.status_message.emit(tr("proxy.generating"))
        sg = self._session_gen
        self._proxy_thread = ProxyBuildThread(Path(self._data.source_path), dst)
        self._proxy_thread.progress_sec.connect(lambda t: self.status_message.emit(f"{tr('proxy.generating')} {t:.0f}s"))
        self._proxy_thread.succeeded.connect(lambda s=sg: self._on_proxy_done(s))
        self._proxy_thread.failed.connect(lambda e: self.status_message.emit(f"proxy: {e}"))
    ...
    def _start_waveform_build(self, session_gen: int, *, has_audio: bool) -> None:
        ...
        self.status_message.emit(tr("waveform.generating"))
    ...
    def _on_keyframe_failed(self, err: str, session_gen: int) -> None:
        ...
        self._keyframes_loaded = False
        self._keyframes_deferred = False
        self.status_message.emit(f"keyframes: {err}")
    ...
    def _undo(self) -> None:
        if not self._data.undo_history:
            self.status_message.emit(tr("undo.status.nothing_to_undo"))
            return
        ...
        self._update_timeline()
        self._save_project()
        self.status_message.emit(tr("undo.status.undo", action=state.description))
        self._refresh_undo_menu_state()
        self._clip_loop_clamp_playhead_if_oob()

    def _redo(self) -> None:
        if not self._data.redo_history:
            self.status_message.emit(tr("undo.status.nothing_to_redo"))
            return
        ...
        self._update_timeline()
        self._save_project()
        self.status_message.emit(tr("undo.status.redo", action=state.description))
        self._refresh_undo_menu_state()
        self._clip_loop_clamp_playhead_if_oob()
    ...
    def can_undo(self) -> bool:
        return bool(self._data.undo_history)

    def can_redo(self) -> bool:
        return bool(self._data.redo_history)
